...
class AnalogView extends WatchUi.WatchFace {
    ...
    public function onLayout(dc as Dc) as Void {
        ...
        if (System.getDeviceSettings() has :doNotDisturb) {
            _dndIcon = WatchUi.loadResource($.Rez.Drawables.DoNotDisturbIcon) as
                BitmapResource;
        } else {
            _dndIcon = null;
        }
    ...
    private function drawHashMarks(dc as Dc) as Void {
        ...
        if (System.SCREEN_SHAPE_ROUND == _screenShape) {
            ...
            for (var i = 0; i < coords.size(); i++) {
                ...
                dc.fillPolygon(
                    [
                        [coords[i] - 1, height - 2] as Array<Float or Number>,
    ...
    public function onUpdate(dc as Dc) as Void {
        ...
        targetDc.fillPolygon(
            generateHandCoordinates(_screenCenterPoint, hourHandAngle, 40, 0, 3)
        );
    ...
    private function drawDateString(
        dc as Dc,
        x as Number,
        y as Number
    ) as Void {
    ...
    public function onPartialUpdate(dc as Dc) as Void {
        ...
        var clockTime = System.getClockTime();
        var secondHand = (clockTime.sec / 60.0) * Math.PI * 2;
        var secondHandPoints = generateHandCoordinates(
            _screenCenterPoint,
            secondHand,
            60,
            20,
