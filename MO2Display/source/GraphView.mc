...
class GraphView extends WatchUi.View {
    ...
    public function initialize(sensor as MO2Sensor, index as Number) {
        ...
        _fonts = [Graphics.FONT_SMALL, Graphics.FONT_MEDIUM, Graphics.FONT_LARGE] as Array<FontDefinition>;
    }
    ...
    public function onUpdate(dc as Dc) as Void {
        ...
        _totalGraph.addItem(_sensor.getData().getTotalHemoConcentration().toFloat());
        _currentGraph.addItem(_sensor.getData().getCurrentHemoPercent().toFloat());

        if (_sensor.getData().getTotalHemoConcentration() != null) {
            var totalString = _sensor.getData().getTotalHemoConcentration().format("%0.2f");
            ...
            _totalGraph.draw(dc, [(width / 4) + 2, 3] as Array<Number>, [width - 3, (height / 2) - 3] as Array<Number>);
            _totalGraph.draw(dc, [(width / 4) + 2 + 1, 3] as Array<Number>, [width - 3 + 1, (height / 2) - 3] as Array<Number>);
            _totalGraph.draw(dc, [(width / 4) + 2, 3 + 1] as Array<Number>, [width - 3, (height / 2) - 3 + 1] as Array<Number>);
            _totalGraph.draw(dc, [(width / 4) + 2 + 1, 3 + 1] as Array<Number>, [width - 3 + 1, (height / 2) - 3 + 1] as Array<Number>);
        }

        if (_sensor.getData().getCurrentHemoPercent() != null) {
            var currentString = _sensor.getData().getCurrentHemoPercent().format("%0.1f");
            ...
            // We should have thick lines eventually... this is a hack for now.
            _currentGraph.draw(dc, [(width / 4) + 2, height / 2] as Array<Number>, [width - 3, height - 3] as Array<Number>);
            _currentGraph.draw(dc, [(width / 4) + 2 + 1, height / 2] as Array<Number>, [width - 3 + 1, height - 3] as Array<Number>);
            _currentGraph.draw(dc, [(width / 4) + 2, height / 2 + 1] as Array<Number>, [width - 3, height - 3 + 1] as Array<Number>);
            _currentGraph.draw(dc, [(width / 4) + 2 + 1, height / 2 + 1] as Array<Number>, [width - 3 + 1, height - 3 + 1] as Array<Number>);
        }
    ...
    //! @return Font to use
    private function pickFont(dc as Dc, string as String, width as Number) as FontDefinition {
        var i = _fonts.size() - 1;
