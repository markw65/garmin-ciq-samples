...
class MapSampleMapView extends WatchUi.MapView {
    ...
    public function initialize() {
        ...
        marker = new WatchUi.MapMarker(new Position.Location({:latitude => 38.85557, :longitude =>-94.79864, :format => :degrees}));
        marker.setIcon(WatchUi.loadResource($.Rez.Drawables.MapPin) as BitmapResource, 12, 24);
        marker.setLabel("Custom Icon");
        map_markers.add(marker);

        marker = new WatchUi.MapMarker(new Position.Location({:latitude => 38.85508, :longitude =>-94.79959, :format => :degrees}));
